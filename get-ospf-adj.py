...
def split_file(input):
    ...
    STARTSTRING_RE = re.compile(r'(Neighbor\sID\s+Pri\s+State\s+Dead\sTime\s+Address\s+Interface)')
...
### REMOVE '--More--         ' from putty line breaks AND GENERATE WORKING FILE
    ...
    with open (input,'r') as rawfile:
        ...
        output_file = str(hostname.group(1)+'.show.ip.ospf.neighbor'+'.csv')

        # Run TextFSM against output block using predefined template
        template = open("get-ospf-adj.template")
        re_table = textfsm.TextFSM(template)
        fsm_results = re_table.ParseText(output)
        print(fsm_results)

        WriteListToCSV(output_file,['Neighbor ID','Priority','State','Dead Time','Address','Interface'],fsm_results)


if __name__ == '__main__':
    if len(sys.argv) != 2:
        # Exit the script
        sys.exit("Usage: ./get-ospf-adj.py <raw IOS config file with additional stuff >")
    input = sys.argv[1]
    split_file(input)